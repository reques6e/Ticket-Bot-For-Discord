...
async def on_ready():
    ...
    if firstRun == True:
        ...
        embed = discord.Embed(title='''**Создать тикет**''', description=f'Нажмите кнопку ниже, чтобы создать тикет!', color=embedColor)
        embed.set_footer(text=f"{footerOfEmbeds} | {bot.user.id}", icon_url=f'{bot.user.display_avatar}')
        nmessage = await tchannel.send(embed=embed, view=TicketCreation(timeout=0))
        ...
        for tickets in allTickets:
            ...
            if tchannel != None:
                ...
                pass
        embed = discord.Embed(title='''**Create a ticket**''', description=f'Нажмите кнопку ниже, чтобы создать тикет!', color=embedColor)
        embed.set_footer(text=f"{footerOfEmbeds} | {bot.user.id}", icon_url=f'{bot.user.display_avatar}')
        try:
            tchannel = bot.get_channel(IDOfChannelToSendTicketCreationEmbed)
...
@ticket.command(name = "create", description="Создать прриватный канал")
async def self(interaction: discord.Interaction, reason: str = 'Unspecified'):
    try:
        ...
        if allowedAcess == True:
            ...
            nchannel = await guild.create_text_channel(f'private-{author.name}', category=category, overwrites=overwrites, topic=f'Причина: {reason} | Создал: {author}')
            embed3 = discord.Embed(description=f'Приватный тикет создан: {nchannel.mention}', color=embedColor)
            ...
            try:
                ...
                await interaction.response.send_message(f'Приватный тикет создан: {nchannel.mention}. Сотрудники ответят в ближайшее время', ephemeral=True)
            embed1 = discord.Embed(title='Тикет создан', description=f'{author.mention} создал приватный тикет', color=embedColor)
            embed1.add_field(name=f'Причина:', value=f'{reason}')
            try:
            ...
            try:
                ...
                message3 = await nchannel.send(f"Тикет создал {author}, Причина: {reason}", view=embedButtons())
            await message3.pin()
            connection = TicketData.connect()
            cursor = TicketData.cursor(connection)
            now = datetime.now().strftime("%m-%d-%Y, %H:%M:%S")
            TicketData.add(connection, cursor, nchannel.id, author.id, f'{now} EST', ticketTypeAllowedToCreatePrivateChannels, "active", message3.id)
            ...
            embed2.add_field(name='Канал:', value=f'{nchannel.mention}', inline=False)
            embed2.add_field(name=f'Причина:', value=f'{reason}', inline=False)
            embed2.add_field(name='Тип:', value='Private', inline=False)
            embed2.set_footer(text=f"{footerOfEmbeds} | {bot.user.id}", icon_url=f'{bot.user.display_avatar}')
            try:
                await syslogc.send(embed=embed2)
            except discord.HTTPException:
                await syslogc.send(f"Тикет создал {author}, Причина: {reason}")
        else:
...
async def self(interaction:discord.Interaction):
    try:
        ...
        embed6.add_field(name= "**__Version__**", value="`v4.3.1`")
